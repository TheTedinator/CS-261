...
int main (int argc, const char * argv[]) {
  ...
  void* one = malloc(sizeof(void*));
  *(int*)one = 1;
  // insertMap(hashTable, "word", one, myCompare, hash2);
  // assert(containsKey(hashTable, "word", myCompare, hash2));
	/*... concordance code goes here ...*/
	fileptr = fopen(filename, "r+");
  char* wordNext;
  while((wordNext = getWord(fileptr))) {
    if(containsKey(hashTable, wordNext, myCompare, hash2) == 0){//if it's not already in the table
      insertMap(hashTable, wordNext, one, myCompare, hash2);//insert it with value one.
    }else{
